...
class DifyClient(BaseClientMixin):
    ...
    def _handle_error_response(
        self, response, is_upload_request: bool = False
    ) -> None:
        ...
        if response.status_code == 401:
            ...
            if (
                is_upload_request
                or "upload" in str(current_url).lower()
                or "files" in str(current_url).lower()
            ):
    ...
    def _send_request_with_files(
        ...
        if self.enable_logging:
            self.logger.info(
                f"Sending {method} file upload request to {endpoint}"
            )
        ...
        if self.enable_logging:
            self.logger.info(
                f"Received file upload response: {response.status_code}"
            )
    ...
    def create_completion_message(
        ...
        if response_mode not in ["blocking", "streaming"]:
            raise ValidationError(
                "response_mode must be 'blocking' or 'streaming'"
            )

        self._validate_params(
            inputs=inputs, response_mode=response_mode, user=user
        )
...
class ChatClient(DifyClient):
    def create_chat_message(
        ...
        self._validate_params(
            inputs=inputs, query=query, user=user, response_mode=response_mode
        )
...
class WorkflowClient(DifyClient):
    ...
    def get_workflow_logs(
        ...
        if created_by_end_user_session_id:
            params["created_by_end_user_session_id"] = (
...
class KnowledgeBaseClient(DifyClient):
    ...
    def create_document_by_text(
        self, name, text, extra_params: Dict[str, Any] | None = None, **kwargs
    ):
        """
